-- ------------------------------------------------------------------------------
--                     Author    : F2 - JPD
--                     Time-stamp: "2021-01-15 06:43:57 jpdur"
-- ------------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- All tables below likely to be useless and thus deleted - 2021-01-11
-- ---------------------------------------------------------------------

-- -- Storage of the values associated to a given DataPoint

-- if exists (select * from dbo.sysobjects where id = object_id(N'DataPointDef') and OBJECTPROPERTY(id, N'IsTable') = 1)
-- drop table DataPointDef
-- GO

-- create table DataPointDef(
...
-- updatedAt    datetime 		not null, -- Sequelize managed fields
-- -------------------------         -- Standard Header
-- Name	      varchar(50)  	  NOT NULL, -- DataPoint ID
-- )

-- -- IS Industry Specific
-- if exists (select * from dbo.sysobjects where id = object_id(N'DataPointDef') and OBJECTPROPERTY(id, N'IsTable') = 1)
-- drop table DataPointDef_IS
-- GO

-- create table DataPointDef_IS(

-- createdAt    datetime 		not null, -- Sequelize managed fields
-- updatedAt    datetime 		not null, -- Sequelize managed fields
-- -------------------------         -- Standard Header
-- DataPointDefID    varchar(36)	NOT NULL, -- the DataPoint Def ID
-- IndustryID	  varchar(36)  	NOT NULL, -- the Industry ID
-- )

-- -- CS Company Specific

-- -- ------------------------------------------------------------------------------------
-- -- This table is used to create for a given company the specific attributes to be used
-- -- ------------------------------------------------------------------------------------
-- if exists (select * from dbo.sysobjects where id = object_id(N'DataPointDef') and OBJECTPROPERTY(id, N'IsTable') = 1)
-- drop table DataPointDef_CS
-- GO

-- create table DataPointDef_CS(

-- createdAt    datetime 		not null, -- Sequelize managed fields
-- updatedAt    datetime 		not null, -- Sequelize managed fields
-- -------------------------         -- Standard Header
-- DataPointDefID    varchar(36)	NOT NULL, -- the DataPoint Def ID
-- Attribute	  varchar(50)  	NOT NULL, -- the Attribute
-- CompanyID	  varchar(36)  	NOT NULL, -- the Company ID
-- )

-- -- Add constraints so that there is only 1 value per date
-- CREATE UNIQUE INDEX DataPointDef_CS_Data on DataPointDef_CS(DataPointDefID,Attribute,CompanyID)
-- GO

-- -- -----------------------------------------------------------------
-- -- Table used when the DataPoint is actually used to enter value
-- -- if not this is left empty
-- -- -----------------------------------------------------------------
-- if exists (select * from dbo.sysobjects where id = object_id(N'DataPointDef') and OBJECTPROPERTY(id, N'IsTable') = 1)
-- drop table DataPoint_ID
-- GO

-- create table DataPoint_ID(
...
-- -------------------------         -- Standard Header
-- DataPointDefID    varchar(36)	NOT NULL, -- the DataPoint Def ID
-- Attribute	  varchar(50)  	, -- the Attribute
-- CompanyID	  varchar(36)  	NOT NULL, -- the Company ID
-- )

-- -- The Attribute may not be populated if the data point is generic
-- -- but it is always associated to a company

-- -- Quid if the attributes are not used and then used later ????
-- -- --> Data Structure might be weak for that specific purpose




-- -- Attribute     varchar(50) 	  ,
-- -- CompanyID     varchar(36)	  , -- ID of company if company specific
