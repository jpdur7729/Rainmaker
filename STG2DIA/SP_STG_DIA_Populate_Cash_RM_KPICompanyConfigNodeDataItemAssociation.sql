-- ------------------------------------------------------------------------------
--                     Author    : FIS - JPD
--                     Time-stamp: "2021-03-22 07:10:10 jpdur"
-- ------------------------------------------------------------------------------

-- 2021-03-18 JPD Similarities and differencies with DIA_Populate_RM_NodeDataItemAssociation to be highlighted
-- 2021-03-22 JPD Cash version... links DataItem = NodeDefIndustry to Node = NodeDef


CREATE or ALTER PROCEDURE [dbo].[STG_DIA_Populate_Cash_RM_KPICompanyConfigNodeDataItemAssociation] ( @HierarchyName as varchar(100) ,@IndustryName as varchar(100) ,@CompanyName as varchar(100) )
as
BEGIN
      ...
      select NEWID() as ID,NC1.Name,NC1.RM_DataItemID as DataItemID,
      	     ...
      	     from NodeDefIndustry NC1 ,Hierarchies h,NodeDef NI,
	     	      RainmakerLDCJP_OAT.dbo.RM_KPICompanyConfigNodeAssociation KCNA
      	     where NC1.IndustryId = @IndustryID
	     	   -- and NC1.CompanyID = @CompanyID
		   and NC1.HierarchyID = @HierarchyID
      	     	   and h.NodeDefID = NC1.ID and NI.ID = h.ParentNodeDefId
		   -- and NC1.level = 1
		   -- and NC1.ID not in (select ParentNodeDefID from Hierarchies) -- Only Final Leaves
