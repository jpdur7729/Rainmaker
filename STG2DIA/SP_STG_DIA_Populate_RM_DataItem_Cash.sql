/* ------------------------------------------------------------------------------
                       Author    : FIS - JPD
                       Time-stamp: "2021-03-21 20:58:05 jpdur"
   ------------------------------------------------------------------------------ */


-- In the case of Cash. There are only DataItems corresponding to NodeDefIndustry
-- No need to upload anything else ==> To be checked as part of the XL Addin

CREATE or ALTER PROCEDURE [dbo].[STG_DIA_Populate_RM_DataItem_Cash] ( @HierarchyName as varchar(100) ,@IndustryName as varchar(100) ,@CompanyName as varchar(100) )
as
BEGIN
      ...
      set @Description = 'Created by STG_DIA_Populate_RM_DataItem_Cash'
      ...
      update NodeDefIndustry set RM_DataItemID = NEWID() where HierarchyID = @HierarchyID and IndustryID = @IndustryID and RM_DataItemID is null
