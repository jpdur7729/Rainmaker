/* ------------------------------------------------------------------------------
                       Author    : FIS - JPD
                       Time-stamp: "2021-03-21 12:58:59 jpdur"
   ------------------------------------------------------------------------------ */

CREATE or ALTER PROCEDURE [dbo].[STG_DIA_Populate_RM_NODE_Company] ( @HierarchyName as varchar(100) ,@IndustryName as varchar(100) ,@CompanyName as varchar(100) )
as
BEGIN
      ...
      -- Step 2: We add the Company Level 1 IF there is am associated company level 2
      --         Only these appear in RM_NODE
      merge into RainmakerLDCJP_OAT.dbo.RM_NODE RM_NODE
      using (
      Select NC.RM_NODE_ID as RMNodeID, NC.NAME as Name,NI.RM_NODE_ID as ParentNodeID,@HierarchyID as KPITypeID,NC.SortOrder as Sequence
			from
