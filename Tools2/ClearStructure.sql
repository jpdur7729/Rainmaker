-- -------------------------------------------------------------------------
--                  Author    : FIS - JPD
--                  Time-stamp: "2021-04-20 10:26:17 jpdur"
-- -------------------------------------------------------------------------
...
BEGIN
      ...
      select * into #ListINDNodes from (select * from RainmakerLDCJP_OAT.dbo.RM_KPI_IND_NodeAssociation
      	     where KPITypeId = coalesce(@HierarchyID,'ZZZ')
      	     	   and KPIIndustryTemplateID = coalesce(@KPIIndustryTemplateID,'ZZZ')) tmp

      select * into #ListCMPNodes from (select * from RainmakerLDCJP_OAT.dbo.RM_KPI_CMP_NodeAssociation
      	     where KPITypeId = coalesce(@HierarchyID,'ZZZ')
      ...
      select * into #ListINDDataItem from (select * from RainmakerLDCJP_OAT.dbo.RM_KPI_IND_NodeDataItemAssociation where NodeAssociationId in (select ID from #ListINDNodes)) tmp
      select * into #ListCMPDataItem from (select * from RainmakerLDCJP_OAT.dbo.RM_KPI_CMP_NodeDataItemAssociation where NodeAssociationId in (select ID from #ListCMPNodes)) tmp
      ...
      delete from RainmakerLDCJP_OAT.dbo.RM_KPI_IND_NodeDataItemAssociation where ID in (select ID from #ListINDDataItem union select NEWID())
      delete from RainmakerLDCJP_OAT.dbo.RM_KPI_CMP_NodeDataItemAssociation where ID in (select ID from #ListCMPDataItem union select NEWID())
      delete from RainmakerLDCJP_OAT.dbo.RM_KPI_IND_NodeAssociation         where ID in (select ID from #ListINDNodes    union select NEWID())
      delete from RainmakerLDCJP_OAT.dbo.RM_KPI_CMP_NodeAssociation         where ID in (select ID from #ListCMPNodes    union select NEWID())
